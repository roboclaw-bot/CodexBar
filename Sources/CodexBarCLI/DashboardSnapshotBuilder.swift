...
enum DashboardSnapshotBuilder {
        ...
        accountPayloads: [ProviderPayload]?) -> DashboardProviderPayload
    {
        ...
        let error = accountPayloads == nil ? rawError : rawError.map {
            ProviderErrorPayload(
                code: $0.code,
                message: identityMode == .none
                    ? "Account usage unavailable"
                    : self.redactEmailShapedText($0.message, mode: identityMode),
                kind: $0.kind)
        }
        let collectedAccounts = accountPayloads?.compactMap { payload in
            self.makeUsageAccount(payload, identityMode: identityMode)
        }
        ...
        return DashboardProviderPayload(
            ...
            accounts: accounts ?? (collectedAccounts?.isEmpty == false ? collectedAccounts : nil),
        ...
        identityMode: DashboardIdentityMode) -> DashboardAccountPayload?
    {
        ...
        let label = identityMode == .none
            ? "Account"
            : self.redactEmailShapedText(account.label, mode: identityMode)
