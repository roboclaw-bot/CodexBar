...
struct CursorCostRefreshRetryTests {
    @Test
    func `forbidden fixture is attempted once across ordinary refresh cycles`() async throws {
        let transport = ProviderHTTPTransportStub { request in
            let url = try #require(request.url)
            let response = try #require(HTTPURLResponse(
                url: url, statusCode: 403, httpVersion: nil, headerFields: nil))
            return (Data(#"{"error":"Cursor is not available in your region."}"#.utf8), response)
        }
        ...
        try await Self.withFixture(fingerprint: "cookie-a", frequency: .fiveMinutes) { fixture in
            fixture.results = Array(repeating: .failure(FixtureError.failed), count: 3)
            fixture.loadOverride = {
                _ = try await fetcher.fetchUsage(cookieHeader: "fixture", since: nil, until: nil)
            ...
            for _ in 0..<3 {
                ...
                await attempts.append(transport.requests().count)
            }
    ...
    @Test
    func `forbidden cost requests cool down without a published snapshot`() async throws {
    ...
    func `expiry permits another attempt and manual refresh can recover immediately`() async throws {
        try await Self.withFixture(fingerprint: "cookie-a", frequency: .fiveMinutes) { fixture in
            ...
            let ttl: TimeInterval = 6 * 60 * 60
            #expect(failure.retryAfter.timeIntervalSince(failure.attemptedAt) == ttl)
            #expect(store.tokenRefreshFailureIsCoolingDown(
                provider: .cursor, now: failure.attemptedAt.addingTimeInterval(5 * 60)))
            #expect(store.tokenRefreshFailureIsCoolingDown(
                provider: .cursor, now: failure.attemptedAt.addingTimeInterval(60 * 60)))
            #expect(store.tokenRefreshFailureIsCoolingDown(
            ...
            store.tokenFetchFailureCooldowns[.cursor] = UsageStore.TokenFetchFailureCooldown(
                attemptedAt: Date().addingTimeInterval(-ttl - 1),
    ...
    private final class Fixture {
        ...
        var forced: [Bool] = []
        var onLoad: ((Int) -> Void)?
        var loadOverride: (() async throws -> CostUsageTokenSnapshot)?
        ...
        init(fingerprint: String?, frequency: RefreshFrequency) throws {
            ...
            self.store._test_tokenUsageSnapshotLoaderOverride = { [weak self] _, force, _, _, _ in
                ...
                self.onLoad?(count)
                if let loadOverride = self.loadOverride { return try await loadOverride() }
