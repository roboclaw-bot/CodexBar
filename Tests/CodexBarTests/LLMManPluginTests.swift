...
struct LLMManPluginTests {
    ...
    @Test(arguments: BundledPluginTestSupport.engines)
    func `normalized daemon addresses request the node and version paths`(
        engine: ProviderPluginEngineKind) async throws
    {
        for (address, _) in LLMManSettingsReaderTests.addresses {
            let snapshot = try await Self.fetch(engine: engine, base: address)
            #expect(snapshot.primary?.usedPercent == 25)
        }
    }
    ...
    {
        let baseURL = try #require(LLMManSettingsReader.baseURL(environment: ["LLMMAN_HOST": base]))
        let transport = ProviderHTTPTransportHandler { request in
            let url = try #require(request.url)
            #expect(url.scheme == baseURL.scheme)
            #expect(url.host == baseURL.host)
            #expect(url.port == baseURL.port)
        ...
        return try await BundledPluginTestSupport.runtime("llmman", engine: engine, transport: transport)
            .fetchUsage(
                settings: ["LLMMAN_HOST": baseURL.absoluteString],
