...
@MainActor
final class APIBalanceScreenshotRenderTests: XCTestCase {
    func test_renderSyntheticBalances() async throws {
        guard let path = ProcessInfo.processInfo.environment["CODEXBAR_BALANCE_SCREENSHOT_DIR"] else {
            throw XCTSkip("Set CODEXBAR_BALANCE_SCREENSHOT_DIR to render synthetic balance cards.")
        }
        let directory = URL(fileURLWithPath: path, isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        for provider in [UsageProvider.atlascloud, .vercel] {
            let snapshot = try await APIBalancePluginTests.fetch(provider.rawValue, engine: .quickJS)
